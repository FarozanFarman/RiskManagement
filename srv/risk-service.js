...
module.exports = async (srv) => {
    ...
    srv.on('READ', 'Risks', async (req, next) => {
        ...
        await Promise.all( res.map( async risk => {
            ...
            const bp = await tx.run(SELECT.one(srv.entities.BusinessPartners).where({ BusinessPartner: risk.bp_BusinessPartner }).columns([ "BusinessPartner", "BusinessPartnerFullName", "BusinessPartnerIsBlocked", "LastName", "FirstName" ]));
            risk.bp = bp;
        }));
        return res;
    });
